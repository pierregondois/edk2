...
/* Metadata handlers. */
STATIC METADATA_HANDLERS  MetadataHandlers[MetadataMaxId] = {
  // MetadataUidId
  {
    MetadataGenerateUid,
    MetadataValidateUid,
  ...
  )
{
  ...
  Status = MetadataHandlers[Id].Generate (
                                  Id,
                                  Token,
                                  Context,
                                  Metadata,
  ...
  )
{
  ...
  for (Id = 0; Id < MetadataMaxId; Id++) {
    Status = MetadataHandlers[Id].Validate ();
    ASSERT_EFI_ERROR (Status);
  }
