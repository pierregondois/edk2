...
STATIC LIST_ENTRY  UidList = INITIALIZE_LIST_HEAD_VARIABLE (UidList);
  ...
  )
{
  ...
  Link = GetNextNode (&UidList, &UidList);
  ...
  if (Link == &UidList) {
    ...
    InsertTailList (&UidList, &Entry->List);
  }
  ...
  )
{
  ...
  Status = MetadataGet (Id, Token, Metadata, MetadataSize);
  if (EFI_ERROR (Status) && (Status != EFI_NOT_FOUND)) {
    ASSERT_EFI_ERROR (Status);
    return Status;
  } else if (Status == EFI_SUCCESS) {
    // Metadata for this (Id/Token) already exists.
    return EFI_SUCCESS;
  } // else (Status == EFI_NOT_FOUND), generate new Metadata.

  // Get the Current Id for this HID/CID/EISAID
  ...
  Status = MetadataAdd (Id, Token, Metadata, MetadataSize);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    return Status;
  }
  ...
  while (TRUE) {
    ...
    while (TRUE) {
      ...
      if (CompareMem (&Metadata0, &Metadata1, sizeof (METADATA_OBJ_UID)) == 0) {
        DEBUG ((
          DEBUG_ERROR,
          "Metadata: Uid: Same Id: EisaId=%d NameId=%a Id=%d \n",
          Metadata0.EisaId,
          Metadata0.NameId,
          Metadata0.Uid
          ));
